...
def cleanup_after_upload_file(res):
    ...
    print("Cleanup cleanup_after_upload_file. Done.")
...
# @pytest.mark.skip
@pytest.mark.asyncio
async def test_websocket_subscribe_to_key_prefix_receives_subscribed_events():
    ...
    async with connect("ws://localhost:8000/ws") as websocket:
        ...
        response = await websocket.recv()  # receive the next message from websocket
        cleanup_after_upload_file(res)  # will trigger event instead of await trigger_event()
