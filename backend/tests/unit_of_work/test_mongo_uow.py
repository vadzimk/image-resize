...
async def test_can_get_project(created_project_doms, project_service):
    ...
    project_dom = await project_service.get_by_object_prefix(created_project_dom.object_prefix)
    assert project_dom is not None
    assert project_dom.id == created_project_dom.id
...
class TestListProjects:
    ...
    async def test_if_nothing_is_specified_returns_ten_first_projects(self, created_project_doms, project_service):
        # async with ProjectsUnitOfWork() as uow:
        #     project_doms = await uow.service.list_projects()

        project_doms = await project_service.list_projects()
        assert len(project_doms) == 10

    # @pytest.mark.skip
    @pytest.mark.parametrize("created_project_doms", [number_of_projects_to_create], indirect=True)
    async def test_when_specified_skip_and_specified_limit_then_returns_projects_left_after_skip(
            self,
        ...
        project_doms = await project_service.list_projects(skip=the_skip, limit=the_limit)
        assert len(project_doms) == expected_left_after_skip

    @pytest.mark.parametrize("created_project_doms", [number_of_projects_to_create], indirect=True)
    async def test_when_specified_limit_and_not_specified_skip_then_returns_projects_limited_by_limit(
            self,
            created_project_doms,
            project_service
    ):
        ...
        project_doms = await project_service.list_projects(limit=expected_limit)
    ...
    async def test_when_specified_skip_and_not_specified_limit_then_returns_projects_left_after_skip_with_default_limit_ten(
        ...
        project_doms = await project_service.list_projects(skip=the_skip)
        assert len(project_doms) == expected_left_after_skip


async def test_can_update_project(created_project_doms, project_service):
    created_project_dom: ProjectDOM = created_project_doms[0]
    ...
    updated_project_dom = await project_service.update_by_object_prefix(created_project_dom.object_prefix, patch)
