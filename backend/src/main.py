        ...
        with s3.listen_bucket_notification(bucket_name=bucket_name,
            ...
            for event in events:
                for record in event.get("Records", []):
                    logger.debug(f'listen_create_delete_s3_events_to_publish: {record["eventName"]}: {record["s3"]["object"]["key"]}')
            ...
            for event in events:
                for record in event.get("Records", []):
                    ...
                    if s3_object_key.rsplit(".")[0].endswith("_original"):
                        ...
                        logger.debug(f"listen_create_s3_events_to_upload_versions: task-id: {task.id}, result_project_id: {result_project_id}")
    except S3Error as err:
        logger.error(f"S3 Error: {err}")
